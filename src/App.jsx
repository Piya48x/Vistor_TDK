...
export default function App(){
  ...
  const onSubmit = async (e) => {
    ...
    if(photoUrl) await supabase.from('visitors').update({ photo_url: photoUrl }).eq('id', data.id)

      // 4️⃣ อัพเดท payload id และ photo_url
    payload.id = data.id
    payload.photo_url = photoUrl

     // 5️⃣ สร้าง QR code (JSON + URL encode)
    const qrString = encodeURIComponent(JSON.stringify(payload))
    const qrDataUrl = await QRCode.toDataURL(qrString)
    await supabase.from('visitors').update({ qr_data: qrDataUrl }).eq('id', data.id)

    // 6️⃣ Reset form
    setForm({
      id_type:'citizen', id_number:'', full_name:'', gender:'', company:'', phone:'',
      contact_person:'', purpose:'', vehicle_plate:'', vehicle_type:'', chip_serial:'', note:''
    })
    setPhotoDataUrl('')
    await loadVisitors()
      ...
      setForm({ id_type:'citizen', id_number:'', full_name:'', gender:'', company:'', phone:'', contact_person:'', purpose:'', vehicle_plate:'', vehicle_type:'', chip_serial:'', note:'' })
      setPhotoDataUrl('')
      await loadVisitors()
